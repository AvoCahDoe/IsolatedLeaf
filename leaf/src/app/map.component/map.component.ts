...
export class CmnMapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private addMarkers(markers: MapMarkerI[]): void {
    ...
    markers.forEach(marker => {
      if (marker.lat != null && marker.lng != null && 
        ...
        console.warn(`[CmnMapComponent] Marker missing valid coordinates:`, marker);
      }
    });
  ...
  private async geocodeMissingMarkers(): Promise<void> {
    const markersToGeocode = this.markersData.filter(m => 
      ...
      (m.addr_city || m.addr_street)
    );

    console.log(`[CmnMapComponent] Starting geocoding for ${markersToGeocode.length} markers...`);

    for (const marker of markersToGeocode) {
      console.log(`[CmnMapComponent] Geocoding marker: ${marker.name || marker.id} (${marker.addr_city})`);

      const coords = await this.geocodingService.geocodeMarker(marker);

      if (coords) {
        marker.lat = coords.lat;
        marker.lng = coords.lng;
        console.log(`[CmnMapComponent] ✅ Geocoded: ${marker.name || marker.id} → [${coords.lat}, ${coords.lng}]`);
      } else {
        console.warn(`[CmnMapComponent] ❌ Failed to geocode: ${marker.name || marker.id}`);
      }

      await new Promise(resolve => setTimeout(resolve, 100)); // Avoid rate limiting
    }

    console.log(`[CmnMapComponent] Geocoding completed.`);
